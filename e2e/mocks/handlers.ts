...
const stripeHandlers = [
  ...
  http.post('https://api.stripe.com/v1/payment_methods', async () => {
    return HttpResponse.json(mockData.stripe.paymentMethods.us_bank_account);
  }),
  
  // Retrieve Payment Method
  http.get('https://api.stripe.com/v1/payment_methods/:id', ({ params }) => {
    return HttpResponse.json(mockData.stripe.paymentMethods.us_bank_account);
  }),
...
const wiseHandlers = [
  ...
  // Get Wise Balances
  http.get('/api/wise/balances', () => {
    return HttpResponse.json([mockData.wise.balance]);
  }),
...
const errorHandlers = [
  ...
  http.post('https://api.stripe.com/v1/payment_intents/error', () => {
  ...
  http.post('/api/wise/transfers/error', () => {
    return new HttpResponse(
      JSON.stringify({
